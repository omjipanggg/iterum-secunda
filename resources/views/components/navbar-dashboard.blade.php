d<nav class="sb-topnav navbar navbar-expand-lg bg-body-tertiary shadow">
    <div class="container-fluid px-12">
        ...
        <ul class="navbar-nav me-auto d-none d-md-block">
            <span class="navbar-text ms-2" id="clock"></span>
        </ul>
                ...
                <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="navbarDropdown">
                    <li>
                        @if (auth()->user()->hasRole(1))
                            <a class="dropdown-item text-end" href="{{ route('master.index') }}">Dashboard</a>
                        @elseif(auth()->user()->hasRole(7))
                            <a class="dropdown-item text-end" href="{{ route('profile.index') }}">Profile</a>
                        @elseif(auth()->user()->hasRole(8))
                            <a class="dropdown-item text-end" href="{{ route('partner.index') }}">Dashboard</a>
                        @else
                            <a class="dropdown-item text-end" href="{{ route('dashboard.index') }}">Dashboard</a>
                        @endif
                    </li>
