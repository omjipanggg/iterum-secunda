...
@foreach ($data->interviewSchedules as $schedule)
	@if ($schedule->proposal->vacancy_id === $vacancy->id)
		...
		@elseif ($schedule->status === 1)
			@php($param=['icon' => 'check-circle', 'variant' => 'success', 'title' => 'Sesi Wawancara diterima!', 'message' => '<p class="m-0">Sesi Wawancara dijadwalkan pada <span class="fw-semibold">'. date_indo_format($schedule->interview_date) .' '. $schedule->interview_time .' WIB</span>, silakan lengkapi data diri Anda melalui <a href="/profile" class="dotted" target="_blank">tautan ini</a>, serta kerjakan serangkaian tes di bawah ini (bila tersedia) sebelum Sesi Wawancara dimulai, semoga beruntung!</p>'])
