...
<div class="container-fluid px-12">
	...
	<div class="row g-3">
		    ...
		    <div class="d-flex flex-wrap align-items-stretch gap-3">
				<div class="card flex-basis-4">
					...
					<div class="card-body position-relative">
						...
						<p class="mb-2 fs-5">Total: <strong>{{ $partners->count() }}</strong> Mitra</p>
						<ul class="square ps-3 m-0 mb-2">
							<li><span class="fw-semibold">Status:</span>
								<ul class="ps-3 m-0">
									@foreach ($partner_status as $status)
									<li>{{ ($status->active) ? 'Aktif' : 'Tidak Aktif' }}: <strong>{{ $status->total }}</strong> Mitra</li>
									@endforeach
								</ul>
							</li>
						</ul>
						<ul class="square ps-3 m-0">
							<li><span class="fw-semibold">Regional:</span>
								<ul class="ps-3 m-0">
				...
				<div class="card flex-basis-4">
					...
					<div class="card-body position-relative">
						...
						<p class="mb-2 fs-5">Total: <strong>{{ $candidates->count() }}</strong> Kandidat</p>
						<ul class="square ps-3 m-0 mb-2">
							<li><span class="fw-semibold">Jenis Kelamin:</span>
								<ul class="ps-3 m-0">
						...
						<ul class="square ps-3 m-0">
							<li><span class="fw-semibold">Pendidikan:</span>
								<ul class="ps-3 m-0">
									<li><strong>Belum diperbarui</strong></li>
								</ul>
							</li>
						</ul>
				...
				<div class="card flex-basis-4">
					...
					<div class="card-body position-relative">
						...
						<p class="mb-2 fs-5">Total: <strong>{{ $vacancies->count() }}</strong> Lowongan Kerja</p>
						<ul class="square ps-3 m-0 mb-2">
							<li><span class="fw-semibold">Status:</span>
								<ul class="ps-3 m-0">
									@foreach ($vacancy_status as $status)
									<li>{{ ($status->active) ? 'Aktif' : 'Tidak Aktif' }}: <strong>{{ $status->total }}</strong> Lowongan Kerja</li>
									@endforeach
								</ul>
							</li>
						</ul>
						<ul class="square ps-3 m-0">
							<li><span class="fw-semibold">Regional:</span>
								<ul class="ps-3 m-0">
									...
									<li>
										{{ $region->region->name ?? 'Belum diperbarui' }}: <strong>{{ $region->total }}</strong> Lowongan Kerja
									</li>
									@endforeach
								</ul>
							</li>
		...
		<div class="col">
			<div class="card">
				<div class="card-body">
					<canvas id="partnersChart" class="m-0"></canvas>
				</div>
			</div>
		</div>
		<div class="col">
		...
		<div class="col">
			<div class="card">
				<div class="card-body">
					<canvas id="vacancyChart" class="m-0"></canvas>
				</div>
			</div>
		</div>
	</div>
