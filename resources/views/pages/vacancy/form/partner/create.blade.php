<form action="{{ route('vacancy.storePartner') }}" method="GET" data-bs-target="#partner_id" class="formAppendToSelect2">
	@method('GET')
	@csrf
	...
	<div class="d-flex flex-wrap gap-2 mb-2">
		...
		<div class="form-select-floating flex-fill">
	    	<select name="region_id" id="region_id_on_modal" class="form-select select2-single-modal" required="">
	    		<option value="" selected="" disabled="">[00] Kosong</option>
	    		@foreach ($regions as $region)
	    			<option value="{{ $region->id }}">[{{ $region->code }}] {{ $region->name }}</option>
	    		@endforeach
	    	</select>
			<label for="region_id_on_modal" class="fw-semibold small">Wilayah</label>
		</div>
	</div>
	...
	<div class="d-flex flex-wrap gap-2 mb-2">
		...
		<div class="form-select-floating flex-basis-6">
			<select name="city_id" id="city_id_on_modal" class="form-select select2-single-server-modal" data-bs-table="cities" required=""></select>
			<label for="city_id_on_modal" class="fw-semibold small">Kota</label>
		</div>
	</div>
	...
	<div class="d-flex flex-wrap gap-2 mb-2">
		...
		<div class="form-select-floating flex-basis-4">
			<select name="total_employees" class="form-select select2-single-modal" id="total_employees_on_modal">
			...
			<label for="total_employees_on_modal" class="fw-semibold small">Total Karyawan</label>
		</div>
		<div class="form-floating flex-basis-4">
	...
	<div class="d-flex flex-wrap gap-2 mt-2">
		<div class="form-floating flex-basis-6">
			<input type="text" name="person_in_charge" class="form-control" id="person_in_charge" autocomplete="off" placeholder="Person In Charge">
			<label for="person_in_charge" class="fw-semibold small">Person In Charge</label>
		</div>
		<div class="form-floating flex-basis-6">
			<input type="text" name="phone_number" class="form-control" id="phone_number" autocomplete="off" placeholder="No. Telepon">
			<label for="phone_number" class="fw-semibold small">No. Telepon</label>
		</div>
	</div>
